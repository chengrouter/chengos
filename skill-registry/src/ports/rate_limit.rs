//! Rate limiting port.
//!
//! The API middleware derives the bucket key (per-IP or per-publisher) and
//! the limit from `RateLimitConfig`; implementations only count. Rate limits
//! are a protection accelerator, not business truth: implementations must
//! fail *open* (allow) when the backing store is unavailable, so a Redis
//! outage degrades protection rather than availability.
