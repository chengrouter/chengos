...
echo "     Grey-clouded records bypass every rule above. A Cloudflare Tunnel"
echo "     creates proxied records itself, so this is already true there."
echo "  2. Make sure the origin cannot be reached around the edge:"
echo "       - Tunnel deployment: nothing listens on the public internet, so"
echo "         just keep inbound 80/443 closed. Do NOT run origin-firewall.sh;"
echo "         it allowlists inbound Cloudflare traffic, which is the wrong"
echo "         shape when there is no inbound connection at all."
echo "       - Reverse-proxy deployment: run ./origin-firewall.sh --apply, or"
echo "         the origin IP still answers direct scans."
[[ $failed -eq 0 ]]
