...
| **skill-importer** | 外部 Skill 导入流水线：抓取 URL/Git 仓库/粘贴文本，经 LLM 解析与生成、`tools/validate_skill_spec` 校验、审批后，由 `tools/write_skill_package` 安全写入 `skills/<name>/`，最终由 SkillFileWatcher 同步入库（默认 `needs_review` + `enabled=false`）。 | 当用户要求「导入这个 Skill / 安装这个 SKILL.md / 把这个仓库转成 ChengOS Skill」时触发 | [SKILL.md](skill-importer/SKILL.md) |
